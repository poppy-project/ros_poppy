...
class Poppy():
    ...
    def __init__(self):
        ...
        self.joints_state_sub = rospy.Subscriber(
            "/poppy/joint_states", JointState, self.state_cb)
        ...
        for k, d in self.poppy_param.iteritems():
            if k != 'joint_state_controller':  # just ignore this one
                ...
                self.joints_pub[k.replace('_position_controller', '')] = rospy.Publisher(
                    "/poppy/" + k + "/command", Float64)
    ...
    def state_cb(self, data):
        for i, n in enumerate(data.name):
            self.joints_state[n] = data.position[
                i]  # there is also velocity and effort
